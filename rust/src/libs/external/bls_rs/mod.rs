...
impl BlsRs {
    pub fn get_public_key(seed: &[u8]) -> Result<Vec<u8>, errors::CryptoError> {
    ...
    fn sign_from_scalar(
        ...
        msg: &[u8],
        dst: &[u8],
    ) -> Result<Signature, errors::CryptoError> {
    ...
    pub fn sign(seed: &[u8], message: &[u8], dst: &[u8]) -> Result<Vec<u8>, errors::CryptoError> {
        let sk = Self::parse_secret_key(seed)?;
    ...
    pub fn verify_signature(
        ...
        msg: &[u8],
        dst: &[u8],
    ) -> Result<(), errors::CryptoError> {
        let pk = PublicKey::deserialize(pk_bytes).map_err(|_| errors::CryptoError::InvalidPoint)?;
        let sig =
            Signature::deserialize(sig_bytes).map_err(|_| errors::CryptoError::InvalidSignature)?;
    ...
    ) -> Result<Vec<u8>, errors::CryptoError> {
        if public_keys.is_empty() {
            return Err(errors::CryptoError::ZeroSizedInput);
        }
    ...
    pub fn aggregate_signatures(signatures: Vec<Vec<u8>>) -> Result<Vec<u8>, errors::CryptoError> {
    ...
    pub fn validate_public_key(public_key: &[u8]) -> Result<(), errors::CryptoError> {
    ...
    fn g1_projective_is_valid(projective: &G1Projective) -> bool {
        !projective.is_identity().into()
            && projective.is_on_curve().into()
            && projective.to_affine().is_torsion_free().into()
    }

    fn g2_affine_is_valid(projective: &G2Affine) -> bool {
        !projective.is_identity().into()
            && projective.is_on_curve().into()
            && projective.is_torsion_free().into()
    }
